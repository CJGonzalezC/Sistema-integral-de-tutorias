...
require_once('lib/fpdf181/fpdf.php');
   ...
   $clave=$_GET['ClaveLibera'];
 
      $query="select * from alumnos as a inner join carreras as c where c.ClaveCarrera=a.ClaveCarrera and NoControl in (select ControlAlumno from subtablaliberaciones where ClaveLibera=$clave )";
...
{//$pdf->Image('../imagenes/upload.png',10,20,25,50);
$pdf->AddPage();
...
//$pdf->Cell(30,6,'ClaveLiberacion: '.$clave,0,0,'C');
 ...
 $pdf->Cell(40,0,'Nombre de Alumno: ');
  $pdf->SetFont('Arial','','11');
 $pdf->Cell(0,0,$datosUsuarios['Nombre']);
 
 $pdf->SetFont('Arial','B','11');
 $pdf->SetXY(20,134);
 $pdf->Cell(25,0,'No Control: ');
  $pdf->SetFont('Arial','','11');
  $pdf->Cell(20,0,$datosUsuarios['NoControl']);
   $pdf->SetFont('Arial','B','11');
   $pdf->Cell(33,0,' de la Carrera de ');
    $pdf->SetFont('Arial','','11');
    $pdf->Cell(0,0,$datosUsuarios['NombreCarrera']);
